...
export const MiFamilia = ({
  ...
  const [leaving_family_id, set_leaving_family_id] = useState<string | null>(
    null,
  );
  ...
  const handle_leave_click = async (
    family_id: string,
    ...
    return (
      <PageContainer>
        ...
        <CardContainer style={{ textAlign: "center", padding: "32px 24px" }}>
          ...
          <Spacer height={8} />
          <TextMuted>
            Eres <strong style={{ color: "#f26841" }}>El Cocinitas 🍳</strong>{" "}
            de esta unidad familiar. Comparte este código con el resto de
            miembros para que puedan unirse:
          </TextMuted>
  ...
  return (
    <PageContainer>
      ...
      <TextMuted style={{ fontSize: 13 }}>
        Perteneces a las siguientes familias (máximo 2):
      </TextMuted>
      <Spacer height={8} />
      ...
      {my_families.length === 0 ? (
        <CardContainer>
          ...
          >
            No formas parte de ninguna unidad familiar. Tu planificación es
            local.
          </TextMuted>
        </CardContainer>
      ) : (
        <PantryInputGrid style={{ gridTemplateColumns: "1fr", gap: 10 }}>
          {my_families.map((fam) => {
            ...
            return (
              <CardContainer
                key={fam.family_id}
                style={{ borderColor: isActive ? "#f26841" : "#32323e" }}
              >
                <FlexRow
                  style={{
                ...
                >
                  <div>
                    <TitleH2 style={{ fontSize: 16, marginBottom: 4 }}>
                      {fam.family_name}
                    </TitleH2>
                    <TextMuted style={{ fontSize: 12 }}>
                      Rol:{" "}
                      <strong
                        style={{
                          color:
                            fam.role === "cocinitas" ? "#f26841" : "#90caf9",
                        }}
                      >
                        {fam.role === "cocinitas"
                          ? "El Cocinitas 🍳"
                  ...
                  >
                    ...
                    <Boton
                      texto={
                        leaving_family_id === fam.family_id
                          ? "Abandonando..."
                          : "Abandonar"
                      }
                      clase_css="btn-sm"
                      ...
                      on_click={() =>
                        handle_leave_click(fam.family_id, fam.role)
                      }
                    />
                  </div>
                </FlexRow>
      ...
      {profile?.active_family_id && suggestions.length > 0 && (
        <>
          <Spacer height={20} />
                  ...
                  {current_role === "miembro" && (
                    <FlexRow style={{ gap: 8 }}>
                      <Boton
                        ...
                        on_click={() =>
                          handle_vote_suggestion(suggestion.id, "like")
                        }
                      />
                      <Boton
                        ...
                        icono={<span style={{ fontSize: "14px" }}>👎</span>}
                        on_click={() =>
                          handle_vote_suggestion(suggestion.id, "dislike")
                        }
                      />
                    </FlexRow>
                  )}
                    ...
                    <>
                      ...
                      <FlexRow style={{ gap: 8 }}>
                        <Boton
                          ...
                          icono={<Check size={14} />}
                          on_click={() =>
                            handle_approve_suggestion(suggestion.id)
                          }
                        />
                        <Boton
                          texto="Rechazar"
                          ...
                          icono={<X size={14} />}
                          on_click={() =>
                            handle_reject_suggestion(suggestion.id)
                          }
                        />
                      </FlexRow>
                    </>
